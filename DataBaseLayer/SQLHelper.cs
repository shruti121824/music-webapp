...
namespace DataBaseLayer
{
    ...
    public sealed class SqlHelper
    {
        ...
        public static int ExecuteNonQuery(string connectionString, string Procedure, int retparams, object[] parameterValues)
        {
            try
            {
                ...
                cmd.Connection.Open();
                foreach (SqlParameter objParam in parameterValues)
                {
                    cmd.Parameters.AddWithValue(objParam.ParameterName.ToString(), objParam.Value.ToString());
                }
                cmd.ExecuteNonQuery();
                connect.Close();
                return 1;
        ...
        public static DataTable ExecuteDataTable(SqlConnection connection, string spName, object parameterValues)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
            ...
            return SqlHelper.ExecuteDataTable(connection, spName, parameterValues);
